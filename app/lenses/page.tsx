...
import { LensCamera, type Lens, type LensFriend } from "./LensCamera";
...
export default async function LensesPage() {
  ...
  const [{ data: lenses }, { data: friends }] = await Promise.all([
    supabase
      .from("lenses")
      .select("slug, name, category, mediapipe_spec")
      .eq("is_active", true)
      ...
      <main className="mx-auto flex min-h-[100dvh] max-w-xl flex-col px-1 py-5 md:py-7">
        ...
        <section className="surface-glass mt-5 p-4">
          <h1 className="font-display text-xl font-semibold">
            <span aria-hidden className="mr-1.5">🤳</span>Lenses
          </h1>
          <p className="mb-3 mt-1 text-sm text-white/55">
            Live AR filters. Face tracking runs entirely on your device — no
            video ever leaves your phone until you choose to share a photo.
          </p>
          <LensCamera
            currentUserId={user.id}
            lenses={(lenses ?? []) as Lens[]}
            friends={(friends ?? []) as LensFriend[]}
          />
        </section>
      </main>
    </AdRails>
